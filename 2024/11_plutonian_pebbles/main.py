...
def test_part(input, blinks, true, part2=False):
    if alt == 1:
        ans = solver(input, blinks, part2=part2)  

    elif alt == 2:
        ans = solver(input, blinks)  
        ans = ans[1] if part2 else ans[0]

    elif alt == 3:
        ans = solver1(input, blinks) if not part2 else solver2(input, blinks)
...
def main():
    test1 = "test_input.txt", 1, 7 
    test2 = "test_input_2.txt", 6, 22
    real = "input.txt", 25
